...
class Junos:
    def __init__(self, module):
        ...
        self.connection_args['target_type'] = 'junos'
        self.connection_args['prompts'] = ['>', ':0%']
        ...
        self.connection_args['cmds_init'] = [
                {"cmd": "set cli screen-length 0", "template": ""},
                {"cmd": "set cli screen-width 1024", "template": ""}
            ]
        self.connection_args['send_cmds_init'] = module['send_cmds_init']
        if self.connection_args['action'] == "fact":
            self.connection_args['cmds'] = [
                {"cmd": "show version", "template": "junos_show_version"}
            ]
        else:
